...
export default function JobDetailPage() {
  ...
  return <main className="mx-auto max-w-5xl space-y-6 pb-16"><Button asChild variant="ghost"><Link href="/jobs"><ArrowLeft className="mr-2 h-4 w-4"/>Back to jobs</Link></Button><header className="rounded-2xl border bg-card p-7"><p className="text-xs font-bold uppercase tracking-wide text-primary">{job.company.name}</p><h1 className="mt-1 text-3xl font-extrabold">{job.title}</h1><p className="mt-2 text-sm text-muted-foreground">{job.locationText||"Location not provided"} · {job.jobType.replaceAll("_"," ")} · {job.minExperience}+ years</p></header>
    <Card><CardHeader><CardTitle>Why you match</CardTitle><p className="text-xs text-muted-foreground">Calculated only from required skills and candidate evidence.</p></CardHeader><CardContent>{!match?<p className="text-sm text-muted-foreground">Candidate evidence match is available to candidate accounts.</p>:match.insufficientEvidence?<p className="text-sm text-amber-600">Insufficient evidence to calculate a percentage.</p>:<><div className="mb-4 flex items-end justify-between"><strong className="text-3xl text-primary">{match.percentage}%</strong><span className="max-w-xl text-right text-xs text-muted-foreground">{match.method}</span></div><div className="divide-y rounded-xl border">{match.rows.map((row:any)=><div key={row.skillId} className="flex items-center justify-between gap-3 p-4"><div><strong>{row.skill}</strong><p className="text-xs text-muted-foreground">Required: {row.requiredProficiency} · Candidate: {row.candidateProficiency || "No passing evidence"}</p><p className="mt-1 text-xs text-muted-foreground">{row.explanation}</p></div>{row.status==="MATCH"?<span className="flex items-center gap-1 text-sm font-semibold text-emerald-600"><CheckCircle2 className="h-4 w-4"/>Match</span>:<span className="flex items-center gap-1 text-sm font-semibold text-amber-600"><XCircle className="h-4 w-4"/>{row.status}</span>}</div>)}</div></>}</CardContent></Card>
