import { ProficiencyLevel, EntityStatus, JobType, UserRoleType } from "@prisma/client";
...
export async function ensureCanonicalJobs(): Promise<{ seeded: boolean; jobsCount: number }> {
  try {
    ...
    const hasAiml = existingJobs.some((job) =>
      job.title.toLowerCase().includes("ai/ml") || job.title.toLowerCase().includes("aiml")
    );

    if (existingJobs.length >= 6 && hasAiml) {
      return { seeded: false, jobsCount: existingJobs.length };
    }
    ...
    if (!dnaCompany) {
      ...
      if (existingAccount?.company) {
        dnaCompany = existingAccount.company as any;
      } else {
    ...
    if (!dnaCompany || !tataCompany || !infosysCompany || !bfCompany) {
      throw new Error("Failed to initialize all canonical employer companies");
    }
    ...
    return { seeded: true, jobsCount: totalNow };
  } catch (err) {
    console.error("[ensureCanonicalJobs] Error seeding canonical data:", err);
    return { seeded: false, jobsCount: 0 };
  }
}
